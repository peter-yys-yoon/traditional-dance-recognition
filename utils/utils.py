...
def accuracy(output, target, topk=(1, 5)):
    ...
    with torch.no_grad():
        ...
        # print('target3', target.view(1, -1).expand_as(pred))
...
def save_checkpoint(state, is_best, ep, filepath=''):
    torch.save(state, os.path.join(filepath, 'checkpoint.pth.tar'))
    torch.save(state, os.path.join(filepath, f'checkpoint_{str(ep).zfill(3)}.pth.tar'))
    if is_best:
